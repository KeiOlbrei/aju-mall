# Mida su aju sinu kohta teada tahab

Sa ei pea seda kõike korraga täitma. **Sa ei peagi seda üksinda täitma** — su aju küsib jooksvalt ise juurde, kui tal midagi puudu jääb.

Siin on teemad. Iga teema juures on kirjas, mida sa võiksid AI-le anda: linke, faile, kuvatõmmiseid — või lihtsalt räägi ja lase Claude'il kirjutada.

**Alusta kahest esimesest.** Edasi otsusta ise, sinu tempo — kas kõik korraga ühel pärastlõunal või iga päev natuke. Tegelikult täitub ülejäänu nädala jooksul niikuinii, töö käigus.

---

## 1. Sinu stiil ja hääl

→ `01-mina/stiil-ja-toon.md`

See on läbiv stiil, mida sa kasutad klientidega suhtlemisel, müügis ja turunduses. Kui su aju seda teab, kõlab kõik, mida ta sinu eest kirjutab, sinu moodi — mitte AI moodi.

**Mida tuua:** oma kodulehe link · kuvatõmmised kujundatud materjalidest · brändi stiiliraamat või juhised, kui need on olemas · 2–3 asja, mille sa oled ise kirjutanud (postitus, kliendikiri, pakkumine) · LinkedIni profiil

> Kolm päris teksti õpetavad su häält rohkem kui pool tundi selle kirjeldamist.

## 2. Sinu ideaalne klient

→ `02-pakkumine/ideaalklient.md`

Kirjelda kliente, kes toovad sulle kõige rohkem rõõmu ja ka kõige rohkem raha. Ja — sama tähtis — neid, kellega sa enam töötada ei taha.

**Mida tuua:** kliendinimekiri või paar viimast projekti · kliendi koduleht, kus on aru saada millega ta tegeleb · tagasisidet ja tänukirju, **klientide enda sõnadega** · kirjavahetust, kus klient enne ostmist küsis või kahtles — **sealt tulevad tüüpilised vastuväited, millele su müügitekstid hiljem vastama peavad** · kust nad sinuni jõuavad.

> Küsimusele „kellega sa ei taha töötada" — et me õpiks neid märkama. Ideaalkliendi infot on vaja turunduse ja müügi suunamiseks.

## 3. Mida sa müüd

→ `02-pakkumine/teenused.md` ja `02-pakkumine/hinnastamine.md`

Su teenused, paketid, hinnad. Ka need hinnad, mis kodulehel ei ole.

**Mida tuua:** hinnakiri · pakkumiste failid · viimased arved · kodulehe teenuste leht

## 4. Kuidas sa töötad

→ `03-protsessid/kliendi-teekond.md` ja `03-protsessid/mis-kordub.md`

Mis juhtub, kui klient ütleb jah — samm-sammult, kuni töö on tehtud. Ja mis kordub iga kliendiga täpselt ühtemoodi.

**Mida tuua:** olemasolevad mallid, checklistid, lepingupõhjad · kirjeldus oma sõnadega

> Siin tuleb välja see, mille sa 4. nädalal esimesena ära annad.

## 5. Mis on sinu eesmärgid ja kuidas sinna jõuda

→ `05-otsused/minu-numbrid.md`

Su numbrid praegu ja see, kus nad peaksid aasta pärast olema. Ja üks number, mida enamik ei ole kunagi välja arvutanud: **mitut klienti sa üldse jõuad teenindada?**

**Mida tuua:** käive, kliendid, töötunnid · mida keskmine klient sulle maksab · mitu klienti sa praegu korraga jõuad · nii täpselt, kui sa tead

> On väga okei, kui sa kõigele praegu vastata ei oska.

## 6. Millist väärtust sa oma kliendile lood? Mis on selle tõestuseks?

→ `02-pakkumine/kliendi-tulemus.md`

Mitte see, mida sa teed, vaid see, mis on kliendi jaoks pärast teisiti. Ja tõestus: sinu klientide edulood.

**Mida tuua:** klientide edulood · tulemused ja numbrid, kui need on olemas · enne-ja-pärast näited · tagasiside ja soovitused · kui kaua see tavaliselt võtab

> Ilma selleta kirjutab su AI juttu sellest, mida sa teed — „sisekujundusteenus" selle asemel, et „korter müüdi kolm nädalat kiiremini".

## 7. Kust kliendid tulevad — ja kus praegu kinni jääb

→ `04-turundus/kanalid-ja-kitsaskoht.md`

Kaks asja, mis koos ütlevad su ajule, mida sulle üldse soovitada.

**Kust nad tulevad:** soovitused, koduleht, sotsiaalmeedia, otsekontakt, reklaam — ja umbkaudu kui palju kust. Enamikul on tegelikult ainult üks kanal, aga seda pole keegi valjusti öelnud.

**Kus kinni jääb:** kas sul on liiga vähe huvilisi? Või on huvilisi, aga müügivestlusi ei tule? Või on kliente, aga sa ei jõua neid teenindada? Või jõuad, aga kasum on väike?

**Mida tuua:** lihtsalt räägi

> See on kõige tähtsam väli kogu ajus. Aju, mis seda ei tea, soovitab sulle sisuturundust siis, kui sul on tegelikult kliente juba rohkem, kui sa jõuad teenindada. Aju, mis teab, ütleb hoopis, mille sa peaksid käest ära andma.
>
> Ja see muutub. Kui kitsaskoht liigub, kirjuta üle — see ongi mõte.

## 8. Mis on juba proovitud

→ `05-otsused/mis-on-proovitud.md`

Mis ei töötanud — turundus, teenused, tööriistad, koostööd. Ja mis on juba otsustatud, mille juurde ei ole vaja tagasi tulla.

**Mida tuua:** lihtsalt räägi

> See on teema, mida keegi ei mõtle ajju panna — ja just see hoiab ära selle, et su AI pakub sulle iga kuu uuesti välja midagi, mille sa juba läbi proovisid.

## 9. Tulevane mina *(vabatahtlik)*

→ `01-mina/tulevane-mina.md`

Kes sa oled, kui see kõik on tööle hakanud. Ja mida sa endale räägid, kui sa kinni jääd — need laused, mis sind peatavad.

**Mida tuua:** ainult iseennast

---

## Nii see päriselt täitub

Sa ei istu maha ja täida kogu aju infoga. **Sa teed oma tavalist tööd koos ajuga, ja aju küsib puuduva ise juurde.**

Näiteks: sa palud kliendile pakkumise koostada. Aju vaatab, et su hinnad ei ole tal olemas, ja küsib. Sa vastad. Ta kirjutab selle õigesse faili ja teeb pakkumise valmis. Järgmine kord ta enam ei küsi.

Nädala pärast on sul ajus info, mis sinna tööd tehes orgaaniliselt koguneb. Ilma et sa oleks pikki vorme täitnud.
